...
# 保存dict
def SaveDict(sDict: dict, sFileName: str):
    sFilePath = getProjectPath() + '/HSData/' + sFileName + '.json'
    with open(sFilePath, 'wb') as wFile:
        pickle.dump(sDict, wFile)
        print(sFileName + '字典保存成功')


# 加载dict
def LoadDict(sFileName: str):
    sFilePath = getProjectPath() + '/HSData/' + sFileName + '.json'
    if os.path.exists(sFilePath):
        with open(sFilePath, 'rb') as rFile:
